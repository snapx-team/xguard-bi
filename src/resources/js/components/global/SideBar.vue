<template>
    <div class="bg-gray-100 overflow-hidden shadow-lg h-full transition duration-150 ease-in-out "
        ...
        <div class="text-center text-white font-semibold flex justify-between items-center relative z-10 bg-purple-700">
            <p v-if="isSideBarOpen" class="px-8"> XGuard BI</p>
        ...
        <div class="py-4 transition duration-150 ease-in-out mt-3"
            ...
            <div class="text-center">
                <i class="fas fa-chart-line fa-3x p-3 text-indigo-800"></i>
                <div v-if="isSideBarOpen">
                    <h1 class="text-2xl tracking-wide text-indigo-800 font-bold my-0">
                        XGUARD BI </h1>
                    <p class="text-xs text-indigo-800">
                        Visualise data, get insights, make decisions, make money! </p>
            ...
            <nav class="mt-8">
                ...
                <div class="-mx-3 pb-2">
                    <router-link :to="{ path: '/bi' }"
                                 class="flex items-center px-3 py-2 rounded-lg"
                                 :class="!isSideBarOpen ? 'mx-6 bg-gray-600 hover:bg-gray-500 justify-center' : 'justify-between hover:bg-gray-200' ">
                        <span v-if="isSideBarOpen" class="text-sm font-medium text-gray-900"><i
                            class="mr-2 fas fa-chart-bar"></i>Dashboard</span>
                        <div v-else><i class="text-white m-2 fas fa-chart-bar"></i></div>
                    </router-link>
                </div>

                <div class="-mx-3 pb-2">
                    <router-link :to="{ path: '/bi/user-profile' }"
                                 class="flex items-center px-3 py-2 rounded-lg"
                                 :class="!isSideBarOpen ? 'mx-6 bg-gray-600 hover:bg-gray-500 justify-center' : 'justify-between hover:bg-gray-200' ">
                        <span v-if="isSideBarOpen" class="text-sm font-medium text-gray-900"><i
                ...
                <div class="space-y-2 mt-8 " :class=" !isSideBarOpen ? 'text-center text-xl': 'text-left text-sm' ">
                    <a href="/"
                       class="text-sm text-gray-600 hover:text-gray-800 transition duration-300 ease-in-out focus:outline-none">
                        <i class="fas fa-sign-out-alt"></i>
                        <span v-if="isSideBarOpen" class="ml-1 font-bold">BACK TO ERP</span>
                    </a>
                </div>
            </nav>
        </div>
